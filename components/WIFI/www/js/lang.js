var EN = {
	...
	setting : {
		header : "SMART PLUG SETTING",
		title : "SET UP CONNECTION",
		caption : "The smart plug needs to connect with our server by WIFI network to send and receive the data. Please set up the smart plug connection by choosing the WIFI name and password (if need) of the WIFI.",
		ssid : "WIFI NAME",
	...
	dashboard : {
		...
		deviceName : {
			...
			sensePlug : "SMART PLUG CONTROLLER"
		},
		sensorName : {
			lightSensor : "LIGHT",
	...
	information : {
		header : "SMART PLUG PROFILE",
		deviceInfoTitle : "DEVICE INFORMATION",
		deviceInfoCaption : "The smart plug is researched and developed by VKIST. The device is managed by UUID",
		...
		deviceUpdateCaption : "The smart plug can automatic update firmware to improve performance and support new devices. User can choose update new firmware automaticaly or by manually",
	...
	actions : {
		...
		setMqttSuccess: "UPDATE MQTT SUCCESSFULLY",
		setMqttFailed: "UPDATE MQTT FAILED"
	}
};
...
var VI = {
	...
	setting : {
		header : "THIẾT BỊ ĐIỀU KHIỂN TỰ ĐỘNG",
		title : "CẤU HÌNH KẾT NỐI INTERNET",
		caption : "Thiết bị điều khiển tự động cần phải được kết nối internet đến máy chủ để gởi và nhận dữ liệu. Vui lòng cấu hình kết nối cho Thiết bị điều khiển tự động bằng cách chọn mạng WIFI và mật khẩu tương ứng với mạng WIFI.",
		ssid : "TÊN MẠNG WIFI",
		...
		selectWiFi: "CHỌN MẠNG WIFI",
		mqttTitle : "CẤU HÌNH KẾT NỐI MQTT",
		saveMqtt: "LƯU"
	},
	dashboard : {
		header : "QUẢN LÝ THIẾT BỊ ĐƯỢC KẾT NỐI VỚI Ổ CẮM THÔNG MINH",
		deviceName : {
	...
	information : {
		header : "THÔNG TIN THIẾT BỊ ĐIỀU KHIỂN TỰ ĐỘNG",
		deviceInfoTitle : "THÔNG TIN",
		deviceInfoCaption : "Thiết bị điều khiển tự động được nghiên cứu và phát triển bởi VKIST. Mỗi thiết bị sẽ có một mã số duy nhất để quản lý",
	...
	actions : {
		...
		setMqttSuccess: "CẬP NHẬT CÀI ĐẶT MQTT THÀNH CÔNG",
		setMqttFailed: "CÀI ĐẶT MQTT KHÔNG THÀNH CÔNG"
	}
};
...
function Language(){
	...
	fn.initSettingPage = function(){
		...
		$("#staSSID option[value='']").html(LANG["setting"]["selectWiFi"]);
		$("#sta-setting-page label[for='staPWD']").html(LANG["setting"]["password"]);
		$("#sta-setting-page label[for='cmdShowPWD']").html(LANG["setting"]["showPassword"]);
		$("#sta-setting-page label[for='cmdShowMqtt']").html(LANG["setting"]["mqttTitle"]);
		...
		$("#mqtt-setup-container .card-title").html(LANG["setting"]["mqttTitle"]);
		$("#mqtt-setup-container label[for='mqttPwd']").html(LANG["setting"]["password"]);
		$("#cmdSetMqtt").html(LANG["setting"]["saveMqtt"]);
	};
